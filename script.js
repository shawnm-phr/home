...
/* home-delta.js — page-specific behavior for the Homepage only.
   Currently empty — no homepage-specific JS beyond phrhome.js. Add
   guarded IIFEs here (early-return if target elements are absent) if
   that changes. */
...
/* hero email capture - new JS additions */
(function(){var form=document.getElementById('hero-email-form');if(!form)return;form.addEventListener('submit',function(e){e.preventDefault();var email=document.getElementById('hero-email-input').value.trim();if(!email)return;window.location.href='https://peopleshr.com/get-in-touch/?email='+encodeURIComponent(email);});}());
